    ...
    private void loadMyOffers() {
        ...
        try {
            ...
            if (currentUser != null && currentUser.getRole() == User.Role.ENTREPRISE) {
                List<Offer> myOffers = offerService.afficherParEntreprise(currentUser.getId());
                for (Offer offer : myOffers) {
                    offersContainer.getChildren().add(createOfferCard(offer));
                }
            } else if (currentUser != null && currentUser.getRole() == User.Role.ADMIN) {
    ...
    private void handleSaveOffer() {
        try {
            ...
            if (currentOffer == null) {
                ...
                User currentUser = SessionManager.getInstance().getUser();
                if (currentUser == null || currentUser.getRole() != User.Role.ENTREPRISE) {
                    showError("Erreur", "Vous devez être connecté en tant qu'entreprise pour publier une offre.");
                    return;
                }
                offer.setEntreprise(currentUser);
