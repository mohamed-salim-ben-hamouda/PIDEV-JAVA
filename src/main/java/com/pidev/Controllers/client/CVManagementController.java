...
public class CVManagementController implements Initializable {
    ...
    @FXML private Label userDisplayNameLabel;
    ...
    private void loadUserInfo() {
        ...
        if (currentUser != null) {
            userDisplayNameLabel.setText(currentUser.getPrenom() + " " + currentUser.getNom());
    ...
    private void loadCvs() {
        try {
            User currentUser = SessionManager.getInstance().getUser();
            if (currentUser != null) {
                cvList.setAll(cvService.afficherParUtilisateur(currentUser.getId()));
            } else {
                cvList.clear(); // Show nothing if not logged in
            }
    ...
    private void handleSubmitCv() {
        ...
        try {
            if (selectedCv == null) {
                ...
                // Use the logged-in user from session
                User currentUser = SessionManager.getInstance().getUser();
                if (currentUser == null) {
